...
bool PDU::decodePDU(const char *pdu)
{
  ...
  unsigned char X;
  overFlow = false;
  SPstart = false; // Fix Bug3: reset surrogate pair state between calls
  ...
  *generalWorkBuff = 0;
  ...
  case DCS_16BIT_ALPHABET_MASK:
    ...
    {
    int pduHexLen = (int)strlen(pdu); // Fix Bug1: bounds guard
    utfoffset = 0;
    while (dulength)
    {
      if (index + 4 > pduHexLen) break; // Fix Bug1: stop if input exhausted
